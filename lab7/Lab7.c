...
void Wyslij(int nr_pociagu, int stan) //wyslij do zajezdni swoj stan
{
wyslij[0]=nr_pociagu;
wyslij[1]=stan;
...
printf("Halo, Witam serdecznie, tu kontroler z zajezdni \n");
if(rand()%2==1){
printf("Mamy piekna pogode oraz wolne tory\n");
}
else{
printf("Niestety  nie mamy  wolnych torów\n");
}
...
status=odbierz[1];
if(status==1){
printf("Pociag %d stoi na zajezdni, przynajmniej nie bedzie katastrofy\n", nr_pociagu);
}
if(status==2){
...
if(paliwo<=REZERWA){
stan=KONIEC_TRASY;
printf("prosze o pozwolenie na zjazd\n");
Wyslij(nr_procesu,stan);
}
else{
